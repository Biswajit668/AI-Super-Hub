    ...
    <div className="w-full max-w-4xl mx-auto p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 shadow-sm dark:shadow-xl space-y-6">
      ...
      {tool.id === 'text-case-converter' && (
        <div className="flex flex-wrap gap-2">
          <button onClick={toUpper} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">UPPERCASE</button>
          <button onClick={toLower} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">lowercase</button>
          <button onClick={toTitle} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">Title Case</button>
          <button onClick={toCamel} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">camelCase</button>
          <button onClick={toKebab} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">kebab-case</button>
          <button onClick={toSnake} className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-indigo-700 dark:text-indigo-300">snake_case</button>
        </div>
      )}
      ...
      {tool.id === 'text-qr-gen' && (
        <div className="space-y-4">
          <input
            ...
            className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500"
          />
          {inputText && (
            <div className="p-4 bg-white rounded-2xl w-fit mx-auto shadow-xl border border-slate-100">
              <img 
      ...
      {tool.id === 'text-base64' && (
        <div className="flex gap-2">
          ...
          <button onClick={handleBase64Decode} className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-white text-xs font-bold">Decode Base64</button>
        </div>
      )}
      ...
      {(generatedResult || (tool.id !== 'text-word-counter' && inputText)) && (
        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-xs font-mono text-indigo-900 dark:text-indigo-300 relative">
          <button
            ...
            className="absolute top-3 right-3 p-1.5 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-[11px] flex items-center gap-1 shadow-sm"
          >
